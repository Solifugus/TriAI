"""
Test script for both DataLink and MockDataLink implementations.
"""
...
def test_real_datalink():
    """Test the real DataLink implementation (requires SQL Server)."""
    print("\n" + "=" * 50)
    print("Testing Real DataLink")
    print("=" * 50)
    
    # Note: This would require actual SQL Server connection
    instances = [{"instance": "localhost\\SQLEXPRESS", "user": "test_user", "password": "test_pass"}]
    
    try:
        db = DataLink(instances, home_db="TestDB", debug=True)
        
        # Test connection (this will likely fail without proper setup)
        print("\n1. Testing connection:")
        result = db.sql_get("SELECT 1 as test_value")
        print(f"Connection test result: {result}")
        
        print("\n✅ Real DataLink connection test passed!")
        return True
        
    except Exception as e:
        print(f"⚠️  Real DataLink test skipped (expected without SQL Server): {str(e)[:100]}...")
        print("This is normal if SQL Server is not available for testing.")
...
def main():
    """Run all tests."""
    print("DataLink Implementation Tests")
    print("============================")
    ...
    # Test real DataLink (may be skipped)
    if test_real_datalink():
        tests_passed += 1
